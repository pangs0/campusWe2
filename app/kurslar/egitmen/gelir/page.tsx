...
import { DollarSign, ArrowLeft, ArrowUp, ArrowDown } from 'lucide-react'
...
export default async function GelirPage() {
  ...
  // Ücretsiz kurslar gelire dahil edilmiyor
  const calcEarnings = (enrollments: any[], price: number, isFree: boolean) => {
    if (isFree || !price) return 0
    return (enrollments?.length || 0) * price * 0.75
  }

  const totalEarnings = courses?.reduce((sum, c) =>
    sum + calcEarnings(c.course_enrollments || [], c.price || 0, c.is_free), 0) || 0

  const thisMonthEarnings = courses?.reduce((sum, c) => {
    if (c.is_free || !c.price) return sum
    const count = c.course_enrollments?.filter((e: any) =>
      new Date(e.created_at) >= thisMonthStart).length || 0
    return sum + (count * (c.price || 0) * 0.75)
  }, 0) || 0
    ...
    <AppLayout user={user} profile={profile}>
      <main className="px-8 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-ink/45 mt-1">{publishedCount} aktif kurs · {totalStudents} toplam öğrenci</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="card bg-green-50 border-green-100">
            ...
            <p className="mono text-xs text-green-600/50 mt-1">Tüm zamanlar · %75 payın</p>
          </div>
          ...
          <div className="card bg-purple-50 border-purple-100">
            ...
            <p className="mono text-xs text-purple-600/50 mt-1">
              {lastMonthStart.toLocaleDateString('tr-TR', { month: 'long', year: 'numeric' })}
            </p>
          </div>
        </div>
        ...
        <div className="card mb-6">
          <p className="mono text-xs text-ink/35 tracking-widest mb-6">AYLIK GELİR TRENDİ</p>
          <div style={{ height: 160, display: 'flex', alignItems: 'flex-end', gap: 12 }}>
            {monthlyData.map((m, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, height: '100%', justifyContent: 'flex-end' }}>
                <p className="mono text-ink/30" style={{ fontSize: 10 }}>
                  ₺{m.earnings > 999 ? (m.earnings / 1000).toFixed(1) + 'k' : m.earnings}
                </p>
                <div style={{
                  width: '100%',
                  height: `${Math.max((m.earnings / maxEarning) * 120, 4)}px`,
        ...
        <div className="card mb-6">
          ...
          {courses && courses.length > 0 ? (
            <div className="space-y-3">
              {[...courses].sort((a, b) => {
                const aE = calcEarnings(a.course_enrollments || [], a.price || 0, a.is_free)
                const bE = calcEarnings(b.course_enrollments || [], b.price || 0, b.is_free)
                return bE - aE
              }).map((c: any) => {
                const students = c.course_enrollments?.length || 0
                const earnings = Math.round(calcEarnings(c.course_enrollments || [], c.price || 0, c.is_free))
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {!c.is_published && (
                            <span className="mono text-xs bg-amber-50 text-amber-600 border border-amber-200 rounded px-1.5 py-0.5 flex-shrink-0">Taslak</span>
                          )}
                          {c.is_free && (
                            <span className="mono text-xs bg-blue-50 text-blue-600 border border-blue-200 rounded px-1.5 py-0.5 flex-shrink-0">Ücretsiz</span>
                          )}
                        </div>
                        <p className="font-serif font-bold text-green-600 flex-shrink-0 ml-2">
                          {c.is_free ? '—' : `₺${earnings.toLocaleString('tr-TR')}`}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-neutral-200 rounded-full overflow-hidden">
                          <div className="h-full bg-brand rounded-full transition-all" style={{ width: `${percentage}%` }} />
                        </div>
                        <span className="mono text-xs text-ink/30 flex-shrink-0">
                          {students} öğrenci · {c.is_free ? 'Ücretsiz' : `₺${c.price}`}
                        </span>
                      </div>
                    </div>
                  </div>
