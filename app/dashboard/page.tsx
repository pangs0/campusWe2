...
export default async function DashboardPage() {
  ...
  const { data: recentUpdates } = await supabase.from('startup_updates').select('*, author:profiles(full_name), startup:startups(name, slug)').order('created_at', { ascending: false }).limit(8)
  const { data: upcomingMeeting } = await supabase.from('meetings').select('*, startup:startups(name, slug)').gte('scheduled_at', new Date().toISOString()).order('scheduled_at', { ascending: true }).limit(1).maybeSingle()
  const { count: pendingTasks } = await supabase.from('tasks').select('*', { count: 'exact', head: true }).eq('assigned_to', user.id).neq('status', 'done')
  const { count: unreadMessages } = await supabase.from('conversations').select('*', { count: 'exact', head: true }).or(`participant1_id.eq.${user.id},participant2_id.eq.${user.id}`)

  const today = new Date().toISOString().split('T')[0]
  let todayGoals: any[] = []
  try {
    const { data } = await supabase.from('daily_goals').select('*').eq('user_id', user.id).eq('date', today).order('created_at')
    todayGoals = data || []
  } catch {}
    ...
    <AppLayout user={user} profile={profile}>
      <main className="px-4 py-6 md:px-8 md:py-10">
        ...
        <div className="flex items-start justify-between mb-6 md:mb-8">
          ...
          <div className="flex items-center gap-2 bg-white border border-neutral-200 rounded-lg px-3 py-2 md:px-4 md:py-2.5">
            <Zap size={14} className="text-brand" />
            <span className="mono text-sm font-medium text-ink">{profile?.karma_tokens || 0}</span>
            <span className="mono text-xs text-ink/40 hidden sm:inline">Karma</span>
          </div>
        </div>

        {/* Yeni kullanıcı — 3 adım */}
        {isNew && (
          <div className="card mb-6 md:mb-8" style={{ borderColor: 'rgba(196,80,10,.2)', background: 'rgba(196,80,10,.02)' }}>
              ...
              {steps.map(s => (
                <Link key={s.key} href={s.href}
                  className={`flex items-center gap-1.5 text-xs rounded-lg px-2.5 py-1.5 border transition-colors ${
                    s.done ? 'text-ink/30 border-neutral-100 bg-neutral-50 pointer-events-none' : 'text-brand border-brand/20 bg-brand/5 hover:bg-brand/10'
                  }`}>
                  {s.done ? <CheckCircle size={11} className="text-green-500 flex-shrink-0" /> : <Circle size={11} className="text-brand flex-shrink-0" />}
                  {s.label}
                </Link>
              ))}
            </div>
        ...
        {/* İstatistik kartları — mobilde 2 kolon */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
          <div className="card">
            <p className="mono text-xs text-ink/35 tracking-widest mb-2">STARTUPLAR</p>
            <p className="font-serif text-2xl md:text-3xl font-bold text-ink">{myStartups?.length || 0}</p>
          </div>
          <div className="card">
            <p className="mono text-xs text-ink/35 tracking-widest mb-2">KARMA</p>
            <p className="font-serif text-2xl md:text-3xl font-bold text-brand">{profile?.karma_tokens || 0}</p>
          </div>
          <div className="card">
            <p className="mono text-xs text-ink/35 tracking-widest mb-2">GÖREV</p>
            <p className="font-serif text-2xl md:text-3xl font-bold text-ink">{pendingTasks || 0}</p>
          </div>
          <div className="card">
            <p className="mono text-xs text-ink/35 tracking-widest mb-2">YETENEKLERİM</p>
            <p className="font-serif text-2xl md:text-3xl font-bold text-ink">{skills?.length || 0}</p>
          </div>
        </div>

        {/* Günlük hedefler + Toplantı — mobilde alt alta */}
          ...
          <div className="card">
            ...
            {upcomingMeeting ? (
              <div className="p-3 rounded-xl" style={{ background: 'rgba(196,80,10,.04)', border: '1px solid rgba(196,80,10,.12)' }}>
                ...
                {upcomingMeeting.startup && <p className="mono text-xs text-brand mt-0.5">{(upcomingMeeting.startup as any).name}</p>}
              </div>
            ) : (
              <div className="text-center py-4">
        ...
        {/* Startuplarım + Topluluk akışı — mobilde alt alta */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
          <div className="lg:col-span-2">
            ...
            {myStartups && myStartups.length > 0 ? (
                  ...
                  <Link key={startup.id} href={`/startup/${startup.slug}`}
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2 ml-4 flex-shrink-0">
                        <span className="mono text-xs text-ink/30 border border-neutral-200 rounded px-2 py-0.5 hidden sm:inline">{startup.stage}</span>
              ...
              <div className="card py-10 text-center" style={{ background: '#faf9f6', border: '1.5px dashed rgba(26,26,24,.15)' }}>
                ...
                <p className="text-sm text-ink/45 mb-5 max-w-xs mx-auto leading-relaxed">Her büyük startup bir eksiklikle başladı. Seninkini buraya taşı.</p>
                <Link href="/startup/new" className="btn-primary inline-flex items-center gap-1.5 text-sm">
                  <Plus size={14} /> İlk startupını oluştur
                </Link>
              </div>
            )}
          </div>
          ...
          <div>
            <h2 className="font-serif text-xl font-bold text-ink mb-4 mt-6 lg:mt-0">Topluluk akışı</h2>
