...
export default function FeedClient({ userId, avatarUrl, fullName, startups, initialPosts }: Props) {
  ...
  return (
    <div>
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-neutral-100">
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => fileRef.current?.click()}
                  className="flex items-center gap-1.5 text-xs text-ink/45 hover:text-brand transition-colors">
      ...
      {posts.length > 0 ? (
        posts.map(post => <FeedPostItem key={post.id} post={post} currentUserId={userId} />)
      ) : (
        <div className="py-16 text-center" style={{ background: '#faf9f6', borderRadius: 12, border: '1.5px dashed rgba(26,26,24,.12)' }}>
          ...
          <p className="text-sm text-ink/45">İlk paylaşımı yapan sen ol.</p>
        </div>
      )}
    </div>
  )
...
function FeedPostItem({ post, currentUserId }: { post: any; currentUserId: string }) {
  const supabase = createClient()
