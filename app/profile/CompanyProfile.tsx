...
import { Edit, Briefcase, Calendar, ArrowRight, Globe, Linkedin, Users } from 'lucide-react'
        ...
        <div className="space-y-4">
          ...
          <div className="card">
            ...
            <div className="space-y-2.5">
              {companyProfile?.company_name && <div><p className="mono text-xs text-ink/30 mb-0.5">ŞİRKET ADI</p><p className="text-sm font-medium text-ink">{companyProfile.company_name}</p></div>}
              {companyProfile?.sector && <div><p className="mono text-xs text-ink/30 mb-0.5">SEKTÖR</p><p className="text-sm text-ink/70">{companyProfile.sector}</p></div>}
              {companyProfile?.company_size && <div><p className="mono text-xs text-ink/30 mb-0.5">ÇALIŞAN</p><p className="text-sm text-ink/70 flex items-center gap-1"><Users size={12} /> {companyProfile.company_size}+</p></div>}
              ...
              {companyProfile?.website_url && <a href={companyProfile.website_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-ink/50 hover:text-brand transition-colors"><Globe size={12} /> Web sitesi</a>}
              {companyProfile?.linkedin_url && <a href={companyProfile.linkedin_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-ink/50 hover:text-brand transition-colors"><Linkedin size={12} /> LinkedIn</a>}
            </div>
            {!companyProfile?.company_name && <Link href="/profile/edit" className="text-xs text-brand hover:underline block mt-2">+ Şirket bilgilerini ekle</Link>}
          </div>

          {profile?.bio && <div className="card"><p className="mono text-xs text-ink/35 tracking-widest mb-2">HAKKINDA</p><p className="text-sm text-ink/60 leading-relaxed">{profile.bio}</p></div>}

          {watchlist.length > 0 && (
            <div className="card">
              <div className="flex items-center justify-between mb-3">
                <p className="mono text-xs text-ink/35 tracking-widest">TAKİP EDİLEN</p>
                <Link href="/sirket" className="text-xs text-brand hover:underline">Tümü</Link>
              </div>
              <div className="space-y-1.5">
                {watchlist.slice(0, 5).map((w: any) => (
                  <Link key={w.id} href={`/startup/${w.startup?.slug}`} className="flex items-center justify-between hover:text-brand transition-colors group">
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/sirket/ilanlar" className="text-xs text-brand hover:underline flex items-center gap-1">Yönet <ArrowRight size={11} /></Link>
            </div>
            {jobListings.length > 0 ? (
              <div className="space-y-2">
                {jobListings.map((j: any) => (
                  <div key={j.id} className="p-3 rounded-xl" style={{ background: 'rgba(26,26,24,.03)', border: '1px solid rgba(26,26,24,.06)' }}>
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-sm font-medium text-ink">{j.title}</p>
                      <span className="mono text-xs bg-green-50 text-green-600 border border-green-100 rounded-full px-2 py-0.5 flex-shrink-0">Aktif</span>
                    </div>
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/sirket/etkinlikler" className="text-xs text-brand hover:underline flex items-center gap-1">Tümü <ArrowRight size={11} /></Link>
            </div>
              ...
              <div className="space-y-2">
                {companyEvents.map((ev: any) => (
                  <div key={ev.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(26,26,24,.03)', border: '1px solid rgba(26,26,24,.06)' }}>
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(196,80,10,.1)' }}>
          ...
          <div>
            ...
            <ProfilePosts userId={user.id} avatarUrl={profile?.avatar_url || null} fullName={profile?.full_name || '?'} startups={[]} initialPosts={posts} />
          </div>
        </div>
      </div>
    </div>
