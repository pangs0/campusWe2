...
type Props = { userId: string; profileData: any }
...
export default function KutuphaneClient({ userId, initialGoals, initialSessions, communityCount }: {
  userId: string
  initialGoals: any[]
  initialSessions: any[]
  ...
  const [weeklyCount, setWeeklyCount] = useState(initialSessions.length)
  const [totalMinutes, setTotalMinutes] = useState(initialSessions.reduce((sum: number, s: any) => sum + (s.duration_minutes || 0), 0))

  // Aktif kullanıcılar
  const [activeUsers, setActiveUsers] = useState<any[]>([])

  // Topluluk stats
  const [communityPomodoros, setCommunityPomodoros] = useState(communityCount)
  ...
  async function saveSession() {
    ...
    await supabase.from('study_status').upsert({ user_id: userId, is_active: false, updated_at: new Date().toISOString() })
    setWeeklyCount((p: number) => p + 1)
    setTotalMinutes((p: number) => p + selectedTime)
    setCommunityPomodoros((p: number) => p + 1)
  ...
  async function addGoal(e: React.FormEvent) {
    ...
    const { data } = await supabase.from('daily_goals').insert({ user_id: userId, title: newGoal.trim(), date: today }).select().single()
